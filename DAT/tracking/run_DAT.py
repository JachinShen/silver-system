import gc
import scipy
from DAT.tracking import guided_backprop
from DAT.tracking.gen_config import *
from DAT.tracking.options import *
from DAT.tracking.bbreg import *
from DAT.modules.model import *
from DAT.tracking.data_prov import *
from DAT.modules.sample_generator import *
...
def forward_samples(model, image, samples, out_layer='conv3'):
    ...
    extractor = RegionExtractor(
        image, samples, opts['img_size'], opts['padding'], opts['batch_test'])
...
def forwordgradients(model, batch_posimgids, batch_possamples, GBP, vector):
    ...
    for theuniqid in uniqueimgid:
        ...
        theuniexps = torch.from_numpy(
            np.stack(theuniexps, 0)).view(-1, 4).numpy()
        theimg = Image.open(img_list[theuniqid]).convert('RGB')
        extractor = RegionExtractor(
            theimg, theuniexps, opts['img_size'], opts['padding'], opts['batch_test'])
        for i, regions in enumerate(extractor):
            regions = Variable(regions)
            if opts['use_gpu']:
                regions = regions.cuda()
...
def train(model, criterion, optimizer, pos_feats, neg_feats, maxiter, posimgids, possamples, negimgids, negsamples, imglist, GBP, in_layer='fc4'):
    ...
    for iter in range(maxiter):
        ...
        posguided_grads = forwordgradients(
            model, batch_posimgids, batch_possamples, GBP, [0, 10])
        sumposguided_grads = torch.sum(torch.abs(posguided_grads) ** 2, 1)
        posgradmean = torch.nn.functional.avg_pool2d(
            sumposguided_grads, [sumposguided_grads.size(1), sumposguided_grads.size(2)])

        posgraddelta = (sumposguided_grads-posgradmean)**2
        posgraddelta = torch.sqrt(torch.nn.functional.avg_pool2d(
            posgraddelta, [posgraddelta.size(1), posgraddelta.size(2)]))
        # posgradscore=1/posgraddelta

        posguided_grads_1 = forwordgradients(
            model, batch_posimgids, batch_possamples, GBP, [10, 0])
        ...
        negguided_grads = forwordgradients(
            model, batch_negimgids, batch_negsamples, GBP, [10, 0])
        ...
        negguided_grads_1 = forwordgradients(
            model, batch_negimgids, batch_negsamples, GBP, [0, 10])
        ...
        spaloss = (1e10*posgraddelta.sum())/(1e10*posgradmean.sum())+(1e10*neggraddelta.sum())/(1e10*neggradmean.sum())\
            + (1e10*posgradmean_1.sum())/(1e10*posgraddelta_1.sum()) + \
            (1e10*neggradmean_1.sum())/(1e10*neggraddelta_1.sum())
...
def run_mdnet(img_list, init_bbox, gt=None, savefig_dir='', display=False):
    ...
    sample_generator = SampleGenerator(
        'gaussian', image.size, opts['trans_f'], opts['scale_f'], valid=True)
    ...
    for i in range(1, len(img_list)):
        ...
        if not success:
            ...
            pos_data = torch.stack(
                pos_feats_all[-nframes:], 0).view(-1, feat_dim)
            neg_data = torch.stack(neg_feats_all, 0).view(-1, feat_dim)

            pos_examples_data = torch.from_numpy(
                np.stack(pos_examples_all[-nframes:], 0)).view(-1, 4).numpy()
            neg_examples_data = torch.from_numpy(
                np.stack(neg_examples_all, 0)).view(-1, 4).numpy()

            pos_imgids_data = torch.from_numpy(
                np.stack(pos_imgids_all[-nframes:], 0)).view(-1, 1).numpy()
            neg_imgids_data = torch.from_numpy(
                np.stack(neg_imgids_all, 0)).view(-1, 1).numpy()
        ...
        # Long term update
            ...
            pos_examples_data = torch.from_numpy(
                np.stack(pos_examples_all, 0)).view(-1, 4).numpy()
            neg_examples_data = torch.from_numpy(
                np.stack(neg_examples_all, 0)).view(-1, 4).numpy()

            pos_imgids_data = torch.from_numpy(
                np.stack(pos_imgids_all, 0)).view(-1, 1).numpy()
            neg_imgids_data = torch.from_numpy(
                np.stack(neg_imgids_all, 0)).view(-1, 1).numpy()
